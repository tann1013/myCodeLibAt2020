     ...
     */
    private function _getDateRangeListByType($type){
        $jList = [];
        //day,week,month
        if(in_array($type, ['day', 'week', 'month'])){
            for ($j=-1;$j>=-7;$j--){
                ...
                if($type=='week'){//@todo 周（近6周、含本周）
                    ...
                    if($j>-6){
                        //上周一
                        $monday =  date('Y-m-d', strtotime("$j monday", time())); //无论今天几号,-1 monday为上一个有效周未
                        //上周日
                        $sunday = date('Y-m-d', strtotime("$j sunday", time())); //上一个有效周日,同样适用于其它星期
                        $cell = [
                            'monday' => $monday,
                            'sunday' => $sunday,
                            'j' => $j
